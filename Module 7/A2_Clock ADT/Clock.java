...
final class Clock {
    ...
    public boolean isEarlierThan(Clock that) { // this = 18:13, that = 19:10
    ...
    // Adds Δ min to the time on this clock. // 18:02 ---> 19:53
    public void toc(int delta) { // 111
        ...
        if (delta > 0) {
            ...
            int minutesNow = this.minutes + minutesToAdd; // 02 + 51 -> 53
            int currentHours = this.hours + hoursToAdd; // 18 + 1 -> 19

            if (minutesNow >= 60) {
                this.minutes = minutesNow - 60; /// final minutes on clock
                currentHours++;
            } else {
